...
if (!$data) {
    header("Location: " . BASE_URL . "/views/jenis_produk/list-jenis_produk.php?error=not_found");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama)) {
        ...
        if ($jenisProduk->update($id, $nama, $deskripsi)) {
            header("Location: " . BASE_URL . "/views/jenis_produk/list-jenis_produk.php?message=updated");
            exit;
        } else {
            $error_message = "Gagal memperbarui jenis produk.";
        }
    }
}
?>
                ...
                <div class="container-fluid px-4">
                    ...
                    <div class="card mb-4">
                        <div class="card-header">
                            <i class="fas fa-boxes me-1"></i> Form Edit Jenis Produk
                        </div>
                            ...
                            <form method="POST"
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" id="nama" name="nama" class="form-control" required
                                        value="<?= htmlspecialchars($_POST['nama'] ?? $data['nama']) ?>">
                                </div>
                                <div class="form-group mb-3">
                                    <label for="deskripsi">Deskripsi</label>
                                    <textarea id="deskripsi" name="deskripsi" class="form-control"
                                        rows="4"><?= htmlspecialchars($_POST['deskripsi'] ?? $data['deskripsi']) ?></textarea>
                                </div>
